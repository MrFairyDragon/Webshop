...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        CountryCode vat = CountryCode.DK;

        for(int i = 3; i < args.length; i++) {
            String[] strings = args[i].split("=");
            switch (strings[0].toUpperCase()) {
                case "--OUTPUT-CURRENCY" -> {too = getEnumFromString(strings[1].toUpperCase(), CurrencyCode.class);}
                case "--INPUT-CURRENCY" -> {from = getEnumFromString(strings[1].toUpperCase(), CurrencyCode.class);}
                case "--VAT" -> {vat = getEnumFromString(strings[1].toUpperCase(), CountryCode.class);}
            }
        }
    ...
    private static float calcPrice(int amount, float price, OrderType type, CurrencyCode from, CurrencyCode too, CountryCode code) {
        switch (type) {
            case ONLINE -> {return price*amount*getTranslationRate(from, too)*getVatRate(code, type);}
            case BOOK -> {return (calcFreight(amount)+(price*amount))*getTranslationRate(from, too)*getVatRate(code, type);}
